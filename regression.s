 ...
 */
    addi x1, x0, -1             // variable used for testing
 	addi x2, x0, 2				// load 2 into register x2
 	nop
 	nop
 	nop
 	addi x2, x2, (-1 & 0xfff)	//result x1 = -1 from line 15 add -1 to x2
 	nop					// x2 = 2
 	nop
 	nop
 	...
 	nop
 	nop
   sltiu  x7, x3, 0           // compare 1 to 0 not less so x7 = 0
   sltiu x7, x7, -1          // compare 0 to -1 but its negative so x7 = 1 
   sltiu x7, x2, 5           // compare two positive numbers so x7 = 1 
   sltiu x7, x2, 1           // comparing 2 to 1 so x7 = 0
   sltiu x7, x1, 2            // comparing a negative and a postive so x7 = 0
 	nop
    xori x8, x0, 0          //  0 and 0 x8 = 0 PASS
    nop
    nop
    nop
    nop
    xori x8, x1, 0         //  0 and 1 x8 = 0xffffffff, PASS
    xori x8, x1, -1        //  x8 = 0   PASS
    xori x8, x0, -1        // x8= 0xffffffff  PASS
    nop
    nop
    nop
    nop
    ori x9, x0,0            // 0 or 0 x9 = 0  PASS 
    ori x9, x0, -1           // 1 or 0 x9 = 1 PASS 
    ori x9, x1, -1          // 1 or 1  x9 = 1 PASS 
    nop
    nop
    nop
    nop
    andi x10, x0, 0         // 0 or 0 x9 = 0  PASS
    andi x10, x1, 0         // 0 or 1 x9 = 0 PASS
    andi x10, x1, -1        // 1 or 1 x9 = 1 PASS 
 	nop
 	nop
 	nop
 	nop
